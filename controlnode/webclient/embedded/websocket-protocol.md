...
| `/ws/ctrl` | PIN required | browser → server (+ `auth_response` back) | `auth_request`, `cmd`, `ack_alert`, `suppress_alert`, `unsuppress_alert`, `set_layout` |
...
`suppressed` is a separate, third axis — an operator's standing decision to stop
showing an alert at all, orthogonal to `acked`/`resolved` rather than a fourth
combination of them. Suppressing targets the *alert*, not the underlying rule: a
rule and its alert are one identity already (retriggering an existing alert's
rule re-asserts the same alert rather than raising a new one), so once
suppressed, the alert stays suppressed through any number of future
re-triggers of that rule — not just until the next one. It clears only when an
operator sends `unsuppress_alert` or the control node restarts; suppression is
held in memory only, the same as every other alert fact, and is never persisted
across a restart.
...
## `suppress_alert` / `unsuppress_alert`

```json
{ "type": "suppress_alert", "id": "rule:CHAMBER-HIGH" }
{ "type": "unsuppress_alert", "id": "rule:CHAMBER-HIGH" }
```

Marks the alert suppressed (or reverses it) in the server registry, which
broadcasts a normal `alert` carrying the new `suppressed` value — not
`alert_acked`, since suppressing is not an acknowledgement. Unlike `ack_alert`,
an unknown id is **not** relayed: there is no stale-row use case for it, since a
client can only hold a locally-suppressed row for an id the server has already
told it about.
