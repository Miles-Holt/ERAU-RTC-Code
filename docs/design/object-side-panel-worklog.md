...
| 07 | Raised alerts as rows, **no inline Ack**; a row opens a second right-side panel scoped to the alarm (plot, time-in-alarm, long description, Ack / Reset / Suppress) | **done, awaiting validation** — server `5ad7106`, client `a3c5239`. Server: `Registry.Suppress`/`Unsuppress`, `Record.Suppressed`/`SuppressedAt`, and the load-bearing fix — `RaiseFor` carries `Suppressed`/`SuppressedAt` forward onto every re-raise, since `evalRules` only raises on a rule's rising edge and always resolves first, guaranteeing every re-trigger would otherwise silently un-suppress. Client: a new "Raised" section on the object panel (`objectSidebar.js`, rows only, no inline Ack) and a new fourth graph-cell-construction site, `WebClient/js/alarmSidebar.js` — a second right-side panel scoped to one alert, with Freeze/Now (item 16) and history-fetch (item 04) wired in from the start. **"Reset" was deliberately NOT built** anywhere, client or server — no design text, worklog entry, or comment thread anywhere gives it a meaning distinct from Ack; guessing at a rocket-test-stand control action's behavior was judged the wrong trade. See open question 5. |
| 07a | A long description on the alert definition — optional `describe "…"` beside `message` in `config/alerts/alerts.alert`, same placeholder interpolation, rendered in the alarm panel | **done, awaiting validation** — server `5ad7106` (new DSL token `TOK_DESCRIBE`, deliberately distinct from the pre-existing `TOK_DESCRIPTION` used by `.chan` files; optional, same placeholder validation as `message`), client `a3c5239` (rendered under the alarm panel's Message section, only when present). |
| 07b | Somewhere to see what is suppressed — a "View suppressed" filter plus a standing count; suppressed alerts have **no row** in the default list (not greyed — hidden) | **done, awaiting validation** (`a3c5239`) — the existing bottom alert bar (`WebClient/js/alerts.js`) hides suppressed rows by default, with a standing "🔕 N" toggle badge (not pulsing — a standing count, not an urgency signal) that reveals them, dashed-and-dimmed rather than blended into an acked row. Suppressed alerts are also excluded from the bar's unacked counts/pulse, matching "clears the red immediately" everywhere, not just the object it came from. |
...
| 09 | A `channels` section on the alert declaring what its plot draws (`plot <ch>`, `line <value-or-channel> "<label>"`); the `bad_data` template becomes one consumer, emitting lines at validMin/validMax | **server half done for ordinary `alert` rules** (`edb05ab`) — new DSL grammar (`TOK_CHANNELS`/`TOK_PLOT`/`TOK_LINE`, distinct from the pre-existing singular `TOK_CHANNEL`), `plot`/`line` validated against known channels at load time, a `line`'s value classified into a fixed number or a live channel reference (never resolved once at raise time — the wire carries the channel name so an operator-settable limit's drawn line always agrees with its current setting, not the value it held when the alert fired). Open question 3 ("what plots by default") was already closed by item 07's fallback to the condition's own channels — this item is additive on top of that, not a replacement. **The `bad_data` template consumer — the design doc's own flagship example — is NOT built and remains genuinely blocked** on open question 2 (`{refDes}` interpolation): a template is authored once and instantiated per node/channel at runtime, with no statically-known channel name to validate `plot`/`line` against at compile time. Item 09 is correctly described as half-done, not fully done, until that question has an answer. **Client half done for ordinary `alert` rules** (`e2ffebd`) — the alarm panel prefers `plotChannels` over the item 07 default when present, and draws `lines` as reference lines: a synthetic Chart.js dataset per line, spanning a fixed ±1,000,000 x-range so it never needs recomputing on pan/zoom/live-follow (the chart's x-scale bounds are always set explicitly, never auto-fit to data). A channel-valued line's y-value is re-read from `channelBuffers` on the same periodic tick the readings section already uses, never resolved once. Required one small, behavior-preserving fix to the SHARED `updateAllGraphs` (every dataset with no `channelBuffers` entry used to have its data blanked every tick, which is dead code for every real channel today but would have made a reference line flicker to invisible immediately) — verified safe for the Graph tab, the object panel and P&ID-embedded graph objects, which all share that function. **Found in review and fixed inline before committing:** the pushed dataset had no `yAxisID` — every other dataset in this codebase sets one explicitly, since the chart's scales are named `y1`..`y6` with no bare `'y'` default; an unset one risked not rendering, or rendering on an independently-auto-scaled axis where the line's position wouldn't correspond to the channel it's meant to sit beside. Set to `'y1'`, matching the default axis an ordinary plotted channel already lands on. **Known limitation, not fixed:** a reference line is very likely not hoverable — `attachProximityTooltip` (shared by every cell) hit-tests cursor distance to a dataset's actual rendered points, and a reference line's two points sit millions of pixels off-canvas by construction, so nothing registers a tooltip hit along the visible segment even though the line itself renders and clips correctly. A correct fix (point-to-segment distance, or a special case for wide-span datasets) means touching shared hover logic well beyond this item — left as a documented gap. As with the server half, the `template every_daqnode` (`bad_data`'s validMin/validMax) case is not built and stays blocked on open question 2. |
...
- **A reference line (item 09) very likely can't be hovered for its tooltip.**
  `attachProximityTooltip` (`graph.js`, shared by every chart cell) hit-tests
  cursor distance to each dataset's actual rendered data POINTS, not to the
  line segment between them. A reference line's two points sit at x =
  ±1,000,000 by construction (see item 09's client-half commit, `e2ffebd`),
  so they're always millions of pixels off-canvas — the line itself draws and
  clips correctly across the visible window, but nothing puts a point close
  enough to the cursor to register a hit anywhere along it. Would need
  point-to-segment distance (or a special case for a wide-span dataset) in
  shared hover code well beyond item 09's own scope — found and left
